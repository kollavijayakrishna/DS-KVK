...
Node* reverseListIterative(Node* head) {
    Node* prev = nullptr; // Initialize pointer to previous node
    Node* current = head; // Start traversal from the head
    Node* next;           // Temporary pointer to store the next node

    // Traverse the list and reverse links iteratively
    while (current != nullptr) {
        next = current->next;  // Store next node temporarily
        current->next = prev;  // Reverse link: point current node to previous
        prev = current;        // Move previous to current
        current = next;        // Move current to stored next node
    }
